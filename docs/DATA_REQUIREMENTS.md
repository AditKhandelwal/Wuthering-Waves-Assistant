...
|---|---|---|
| `character_stat_curves.json` | Per-character level-1 base HP/ATK/DEF + universal growth-ratio curve by (level, breachLevel) | ✅ Confirmed — cross-checked against known Sanhua lvl 90 values. Built via `scripts/fetch_character_stat_curves.py` (added 2026-08-08 — previously hand-built with no rerunnable script, which is why the 2026-08 patch's 4 new characters were missing stats until this was written). Source: `Arikatsu/WutheringWaves_Data`'s `BinData/role/roleinfo.json` (role → PropertyId) + `BinData/property/baseproperty.json` (PropertyId+level → stats, confirmed PropertyId == roleGbId for every character checked) + `BinData/property/rolepropertygrowth.json` (the universal growth curve). That repo renames its default branch to the current game version (e.g. `"3.5"`) each patch — the script resolves it dynamically via the GitHub API rather than hardcoding it |
...
| `weapon_catalog.json` | Name/icon/rarity/rank-scaled passive text for every weapon, keyed by gbId (`scripts/fetch_weapon_catalog.py`) | ✅ dotgg.gg `type=weapons` endpoint, filtered to gbIds present in `weapon_stat_curves.json`'s `baseAtk` (a display entry with no computable ATK is useless). Merged in `loadWeaponCatalog()` (`frontend/src/lib/weapons.ts`) as a fallback under Kuro's own per-character recommended-weapon text, which wins when both exist. 117/118 weapon gbIds have display data from one source or the other as of 2026-08-07; `21010045`/`21020045`/`21050045` have neither (see `.claude/rules/api.md`) |
...
- **Kuro's guide text and the game8.co-sourced `echo_catalog.json` punctuate some echo names differently** (found 2026-08-07 investigating why Suisui's build screen wasn't pre-filling a default echo) — e.g. Kuro's `"Reminiscence - Nightmare: Adam Smasher"` vs the catalog's `"Reminiscence: Nightmare Adam Smasher"`; `"Twin Nova: Nebulous Cannon"` vs `"Twin Nova - Nebulous Cannon"`. Combined with a second bug (the signature-echo lookup in `BuildScreenPage.tsx` only searched `echoCatalog.byCost[4]`, but several signature echoes — including Suisui's "Forbidden Bastion" — are cost 3), this silently broke the default-echo pre-fill for 7 of 58 characters. Fixed: `findEchoByName()` in `frontend/src/lib/echoes.ts` now searches all cost tiers through a `normalizeEchoName()` comparison (strip punctuation, lowercase) instead of an exact string match.
